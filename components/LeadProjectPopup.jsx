        ...
        {open && !meetingOnly && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-2xl rounded-3xl border border-white/15 bg-[#0b1116] p-6 md:p-8"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="mb-5 flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-cyan-300">Project Intake</p>
              ...
              {!submitted ? (
                <form onSubmit={handleSubmit} className="grid gap-4">
                  ...
                  <div className="flex flex-wrap gap-3 pt-2">
                    ...
                    <button
                      ...
                      className="rounded-xl border border-white/25 px-5 py-2 text-sm font-semibold text-white transition hover:bg-white/10"
                    >
                ...
                <div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/10 p-5 text-sm text-emerald-100">
        ...
        {(meetingOnly ? open : meetingOpen) && (
          ...
          >
            <motion.div
              ...
              onClick={(event) => event.stopPropagation()}
              className="h-[85vh] w-full max-w-5xl overflow-hidden rounded-3xl border border-white/15 bg-[#090f14]"
            >
...
function InputField({ label, ...props }) {
  return (
    <label className="grid gap-2 text-sm text-gray-200">
      ...
      <input
        {...props}
        className="w-full rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-white outline-none transition focus:border-cyan-300/60"
      />
    </label>
  );
}
